                                                                ...
                                                                random(NULL), bondcount(NULL), recreate(NULL)
{
  ...
  //if (neutral_type < 1 || neutral_type > atom->ntypes || ctcf_left < 1 || ctcf_left > atom->ntypes || ctcf_right < 1 || ctcf_right > atom->ntypes)
...
void FixExtrusion::post_integrate()
{
  ...
  for (int numbnd = 0; numbnd < nbondlist; numbnd++)
  {
    ...
    if (
        ...
        (utils::strmatch(ctcf_left,"-"+std::to_string(atom->type[atom->map(tag[i1] - 1)])+"-") ||
         utils::strmatch(ctcf_right,"-"+std::to_string(atom->type[atom->map(tag[i1] - 1)])+"-") ||
         utils::strmatch(ctcf_left_right,"-"+std::to_string(atom->type[atom->map(tag[i1] - 1)])+"-")) &&
        (!utils::strmatch(ctcf_left,"-"+std::to_string(atom->type[atom->map(tag[i1] - 1)])+"-") ||
         ...
         through_prob > random->uniform()))
    {
      ...
      if (
          ...
          (utils::strmatch(ctcf_left,"-"+std::to_string(atom->type[atom->map(tag[i2] + 1)])+"-") ||
           utils::strmatch(ctcf_right,"-"+std::to_string(atom->type[atom->map(tag[i2] + 1)])+"-") ||
           utils::strmatch(ctcf_left_right,"-"+std::to_string(atom->type[atom->map(tag[i2] + 1)])+"-")) &&
          (!utils::strmatch(ctcf_right,"-"+std::to_string(atom->type[atom->map(tag[i2] + 1)])+"-") ||
           through_prob > random->uniform()) &&
          (!utils::strmatch(ctcf_left_right,"-"+std::to_string(atom->type[atom->map(tag[i2] + 1)])+"-") ||
           through_prob > random->uniform()))
